...
#define TAG "de.appwerft.applifecycle"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> Applifecycle_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("Applifecycle.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::ApplifecycleBindings::lookupGeneratedInit(
...
static void Applifecycle_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(Applifecycle_getBinding)->GetFunction());
}

static void Applifecycle_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::ApplifecycleBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry ApplifecycleBinding = {
	"de.appwerft.applifecycle",
	Applifecycle_init,
	Applifecycle_dispose
};

// Main module entry point
extern "C" JNIEXPORT void JNICALL
Java_de_appwerft_applifecycle_ApplifecycleBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("de.appwerft.applifecycle", &ApplifecycleBinding);
	titanium::KrollBindings::addExternalLookup(&(::ApplifecycleBindings::lookupGeneratedInit));
}
